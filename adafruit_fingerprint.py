...
class Adafruit_Fingerprint:
    ...
    def _get_packet(self, expected: int) -> List[int]:
        ...
        if (not res) or (len(res) != expected):
            ...
            print("RuntimeError in adafruit file (_get_packet(), line 343)")
            print("Possible error due to loose wires, try continue.")
            return "skip"   # (I've commented out the runtime error and replaced it 
                            # so that the program keeps running when the breadboard wires 
                            # wiggle around)
