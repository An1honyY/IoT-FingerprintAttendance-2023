...
def enroll_finger(location):
    ...
    if i == adafruit_fingerprint.OK:
        print("Created")
        save_user(get_name(location))   # Ask user for name and save to file
    else:
...
def get_name(location):
    ...
    name_dict = {location: name}    #location is the finger_id index
    return name_dict

# Write users to file
def save_user(name_dict):
    file = open("users.txt" , "a")
    #file.write(str(name_dict)+"\n")
    file.write(json.dumps(name_dict)+"\n")
    
def reset_users():
    open("users.txt", "w").close()  # write clears the file contents
...
def read_users():
    ...
    for line in file:
        user = json.loads(line) # read each line from the file as a dict

        if count == 0:
            users_dict = user
        else:
            users_dict.update(user) # merge dicts into one large

        count += 1

    return users_dict

# Get specific user name using index
def get_user(users_dict, index):
    name = users_dict.get(str(index))
    return name
